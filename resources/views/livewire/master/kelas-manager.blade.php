<div>
    ...
    <div class="d-md-flex justify-content-between">
        <h2 class="mb-3"><span class="text-muted fw-light">Master /</span> Kelas</h2>
    ...
    <div class="card">
        ...
        <div class="table-responsive text-nowrap" class="position-relative">
            ...
            <tr class="border-top">
                ...
                <th style="width:30%;" class="sort" wire:click="sortOrder('prodi.name')">Prodi {!! $sortLink !!}</th>
                <th class="sort" wire:click="sortOrder('kelas.name')">Nama Kelas {!! $sortLink !!}</th>
            ...
            <tr>
                ...
                <td class="border-start">{{ $kelas->prodi->name ?? '' }}</td>
    ...
    <div wire:ignore.self class="modal fade" id="KelasModal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" kelas="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="mb-3">
                    @inject('prodi', 'App\Models\Prodi')
                    <label class="form-label">Program Studi</label>
                    <select wire:model="prodi_code" class="form-select @error('prodi_code') is-invalid @enderror">
                        <option value="">-- Choose --</option>
                        @foreach($prodi::orderBy('code')->get()->pluck('name','code') as $key => $val)
                        <option value="{{ $key }}" @if($key==$prodi_code) selected @endif>{{ $val }}</option>
                        @endforeach
                    </select>
                    @error('prodi_code')
                    <div class="invalid-feedback">
